...
describe('ContentClassifier', () => {
  ...
  describe('classifyFile', () => {
    ...
    it('classifies office files correctly', () => {
      const cases = ['report.docx', 'spreadsheet.xlsx', 'presentation.pptx', 'notes.txt', 'readme.md']
      for (const file of cases) {
  ...
  describe('statistics', () => {
    it('calculates category statistics by size', () => {
      const files = [
        ...
        { path: 'c.pdf', sizeBytes: 2000, extension: '.pdf', category: 'documents', mimeType: null },
        { path: 'd.jpg', sizeBytes: 1500, extension: '.jpg', category: 'images', mimeType: null }
      ] as const

      const stats = classifier.getCategoryStatistics(files as unknown as readonly import('@shared/types').FileInfo[])
      expect(stats['images']).toBe(2500)
      expect(stats['videos']).toBe(5000)
      expect(stats['documents']).toBe(2000)

      const counts = classifier.getFileCountByCategory(files as unknown as readonly import('@shared/types').FileInfo[])
    ...
    it('determines dominant category by size', () => {
      ...
      const dominant = classifier.getDominantCategory(files as unknown as readonly import('@shared/types').FileInfo[])
