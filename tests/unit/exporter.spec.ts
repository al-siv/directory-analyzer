...
function makeScanResult(format: ScanOptions['outputFormat']): ScanResult {
  ...
  const dirs: DirectoryInfo[] = [
    { path: '/test/photos', sizeBytes: 5000, fileCount: 2, lastScanned: Date.now(), errorMessage: null },
    { path: '/test/videos', sizeBytes: 10000, fileCount: 1, lastScanned: Date.now(), errorMessage: null }
  ]
...
async function cleanup(): Promise<void> {
  try { await unlink(TMP_FILE) } catch { /* ignore */ }
}
...
describe('exportResults', () => {
  ...
  it('throws on unsupported format', async () => {
    const result = makeScanResult('text')
    await expect(exportResults(result, 'xml' as unknown as 'text', TMP_FILE)).rejects.toThrow('Unsupported')
  })
})
