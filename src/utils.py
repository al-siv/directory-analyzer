...
def get_direct_files(directory: Path) -> Iterator[Path]:
    ...
    try:
        ...
        logging.getLogger('directory_analyzer').warning(
            f"Cannot access directory {directory}: {e}"
        )


def get_subdirectories(directory: Path, include_hidden: bool = False) -> Iterator[Path]:
    ...
    try:
        for item in directory.iterdir():
            if item.is_dir():
                if include_hidden or not is_hidden_directory(item):
                    yield item
    except (PermissionError, OSError) as e:
        logging.getLogger('directory_analyzer').warning(
            f"Cannot access directory {directory}: {e}"
...
def validate_output_path(output_path: Path) -> Optional[str]:
...
class ProgressReporter:
    ...
    def __init__(self, total: Optional[int] = None, prefix: str = "Progress"):
        self.total = total
        self.prefix = prefix
        self.current = 0
