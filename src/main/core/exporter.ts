...
function generateTextExport(result: ScanResult): string {
  ...
  if (result.errorCount > 0) {
    lines.push(`  Access Errors:      ${String(result.errorCount)}`);
    lines.push(
      `  Success Rate:       ${((1 - result.errorCount / result.totalScanned) * 100).toFixed(1)}%`
    );
  }
  lines.push('');
...
function generateCsvExport(result: ScanResult): string {
  ...
  lines.push(`# Total Directories,${String(stats.totalDirectories)}`);
  lines.push(`# Total Files,${String(stats.totalFiles)}`);
  lines.push(`# Total Size (bytes),${String(stats.totalSizeBytes)}`);
...
function generateJsonExport(result: ScanResult): string {
  ...
  const contentAnalysis: Record<
    string,
    { sizeBytes: number; percentage: number; fileCount: number; displayName: string }
  > = {};
    ...
    summary: {
      ...
      successRate:
        result.totalScanned > 0
          ? (result.totalScanned - result.errorCount) / result.totalScanned
          : 0,
    },
